...
function printSummary(groupedWarnings, groupedErrors) {
  // Get number of error/warn messages
  const countEntries = (entries) =>
    Object.entries(entries).reduce((acc, [, v]) => acc + v.length, 0)
  ...
  const errCount = countEntries(groupedErrors)
  const warnCount = countEntries(groupedWarnings)
